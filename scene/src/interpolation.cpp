...
vec3 interpolationdir(float t, numarray<vec3> const& key_positions, numarray<float> const& key_times)
{
    // Find idx such that key_times[idx] < t < key_times[idx+1]
    int idx = find_index_of_interval(t, key_times);
    float t1 = key_times[idx];
    float t2 = key_times[idx+1]; // = t_i
    vec3 p = key_positions[idx+1]- key_positions[idx];

    p = linear_interpolation(t, t1, t2, key_positions[idx] - key_positions[idx - 1], p);
