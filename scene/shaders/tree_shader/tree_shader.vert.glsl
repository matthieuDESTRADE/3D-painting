...
void main()
{
	...
	vec3 normal3 = vertex_normal.xyz;
	vec3 position3 = vertex_position.xyz;



	// get a turbulent 3d noise using the normal, normal to high freq
 noise = 0.1 *  -.12 * turbulence( .5 * (normal3+vec3(0.05*time,0.05*time,0.05*time)));
  // get a 3d noise using the position, low frequency
  float b = 5.0 * pnoise( 0.05 * position3, vec3( 100.0 ) );
  // compose both noises
