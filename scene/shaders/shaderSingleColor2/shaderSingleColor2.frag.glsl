...
void main()
{
	vec4 c1 = vec4(0.24, 0.20, 0.0, 1.0);
	vec4 c2 = vec4(0.5,0.5,0.0,1.0);
	FragColor = (abs(fragment.position.x) + abs(fragment.position.y) + abs(fragment.position.z))/2 * (c1-c2)/0.63 + c2;
