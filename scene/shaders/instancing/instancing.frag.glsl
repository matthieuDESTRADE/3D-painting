...
void main()
{
	...
	float t= exp(-0.1*length(camera_position-fragment.position));

	float x = fragment.position.x;
	float y = fragment.position.y;

	if (y<x/1.5 + 0.3 && y>2*x-1 && y>x-0.5 && y<x/6 + 0.4 && y>-0.35 && x>-0.2){
		float l = min(min(min(min(min(x/1.5 + 0.3-y,y-2*x+1), y-x+0.5f),x/6+0.4f-y), y+0.35), x+0.2);
		if (0.45<color_object[0] && color_object[0]<0.85 && 0.45<color_object[1] && color_object[1]<0.85 && 0.15<color_object[2] && color_object[2] < 0.65){
		color_object = color_object + 2*sqrt(min(l,1))*vec3(-0.6,-0.5,-0.15);
	}

	}

	if (length(fragment.position-femme + vec3(-0.05f,-0.05f,0))<.11f){
		float l = .11f-length(fragment.position-femme+ vec3(-0.05f,-0.05f,0));
		l*=l*1000;
		l = min(l,0.1f);
		if (color_object[0] > 0.6){color_object = color_object - 2*sqrt(min(l,1))*vec3(0.4,0.3,-0.1);}		
		else {color_object = color_object -2*sqrt(min(l,1))* vec3(0.7,0.4,0.0);}
		}
	vec3 color_shading = color_object * t + (1-t)*vec3(0.,0.,0.2) ;
